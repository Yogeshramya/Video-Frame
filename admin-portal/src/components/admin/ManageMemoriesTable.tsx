...
import { Eye, Trash2, ShieldAlert, RefreshCw, Loader2, AlertCircle } from 'lucide-react';
...
export default function ManageMemoriesTable() {
  ...
  const triggerRecompilation = async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Compiler process failed.';
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to update memory status.';
      setActionError(errorMessage);
    }
  };
